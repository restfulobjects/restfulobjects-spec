...
The domain object representation xref:section-c/chapter-14.adoc[§C14].4 is the most important representation within Restful Objects; from it various sub-resources can be accessed.
The most immediate such sub- resources are for:

* each property of the object (xref:section-c/chapter-16.adoc[§C16].4)
* each collection of the object (xref:section-c/chapter-17.adoc[§C17].5), and
* each action of the object (xref:section-c/chapter-18.adoc[§C18].2).
...
The representation returned by invoking the action will either be a scalar value xref:section-b/chapter-12.adoc[§B12], a representation of the returned domain object xref:section-c/chapter-14.adoc[§C14].4, or a list xref:section-b/chapter-11.adoc[§B11] (linking to multiple domain object resources xref:section-c/chapter-14.adoc[§C14]) As well as providing access to sub-resources, the domain object resource also allows multiple properties to be updated, and objects to be deleted in a single transaction.
...
There are six such resources (xref:section-d/chapter-21.adoc[§D21].2, xref:section-d/chapter-22.adoc[§D22].2, xref:section-d/chapter-23.adoc[§D23].2, xref:section-d/chapter-24.adoc[§D24].2, xref:section-d/chapter-25.adoc[§D25].2, and xref:section-d/chapter-26.adoc[§D26].2).
...
The representations defined by Restful Objects are RESTful in the sense that they provide relevant links to other resources that the client can follow: this is Restful Objects' support for HATEOAS xref:section-e/chapter-28.adoc[§E28].1. Depending on the nature of the domain object being represented, links may or may not be present:

* to properties xref:section-c/chapter-16.adoc[§C16].1 and collections xref:section-c/chapter-17.adoc[§C17].1 of the object
* to actions xref:section-c/chapter-18.adoc[§C18].1on the object
* to update all properties xref:section-c/chapter-14.adoc[§C14].2 of the object
* to persist the object xref:section-b/chapter-09.adoc#_9-1-http-post[§B9]
* to delete the object xref:section-c/chapter-14.adoc[§C14].3 The following sections describe how the different types of domain object will result in the presence or absence of specific links.
...
And if object deletion xref:section-c/chapter-14.adoc[§C14].3 is supported by the implementation, then the delete object link will also be present.
...
See xref:section-e/chapter-32.adoc[§E32].1 for a code sketch as to how this might be accomplished.
...
Objects/
{DType}/
{IID}    Object Property xref:section-c/chapter-14.adoc[§C14].4


Objects/
{DType}/
{IID}/ Properties/
{Property}    Object Collection xref:section-c/chapter-16.adoc[§C16].4


Objects/
{DType}/
{IID}/ Collections/
{Collection}    Object Action xref:section-c/chapter-17.adoc[§C17].5


Objects/
{DType}/
{IID}/ Actions/
{Action}    Object Action Invoke xref:section-c/chapter-18.adoc[§C18].2
...
The HTTP GET method is the most widely supported across the various resources, and is used to obtain a summary representation of an object xref:section-c/chapter-14.adoc[§C14].4 (e.g. a Customer instance), or detailed information about a specific property of an object xref:section-c/chapter-16.adoc[§C16].4 (e.g. Customer.firstName) or about a specific collection xref:section-c/chapter-17.adoc[§C17].5 (e.g. Customer.orders).
In addition, HTTP GET is used to obtain a representation of an object action xref:section-c/chapter-18.adoc[§C18].2, such as the Customer's placeOrder() action.
...
So, properties can be set to a new value using HTTP PUT xref:section-c/chapter-16.adoc[§C16].2, or can be set to null using HTTP DELETE xref:section-c/chapter-16.adoc[§C16].3. Modifying multiple properties is accomplished using an HTTP PUT to the object resource xref:section-c/chapter-14.adoc[§C14].2. For collections things are a little more involved because the HTTP method to use depends upon the collection's semantics.
...
In this case the HTTP PUT xref:section-c/chapter-17.adoc[§C17].2 is used; if the object exists then the request to add it is ignored, so this is idempotent.
If the collection does allow duplicates (in other words, it follows ‘List’ semantics) then HTTP POST xref:section-c/chapter-17.adoc[§C17].3 is used.
In either case references are removed from the collection using HTTP DELETE xref:section-c/chapter-17.adoc[§C17].4. Actions are invoked through the '/invoke’ sub-resource.
The method used depends on the action's semantics: if the action is idempotent, then PUT xref:section-c/chapter-19.adoc[§C19].2 is used, otherwise POST xref:section-c/chapter-19.adoc[§C19].3 is used.
However, there is a further special case for actions: if the action is query-only and so makes no changes to persisted objects at all , then Restful Objects allows HTTP GET xref:section-c/chapter-19.adoc[§C19].1 to be used to invoke the action.
...
Services/{ServiceId}    Service Action xref:section-c/chapter-17.adoc[§C17].5

Services/{ServiceId}/ Actions/{Action}    Service Action Invoke xref:section-c/chapter-18.adoc[§C18].2
...
Representation type Indicates a representation of homepage the start page xref:section-a/chapter-04.adoc[§B4] user the user requesting the resource xref:section-b/chapter-06.adoc[§B6] version the version of the spec and implementation xref:section-b/chapter-08.adoc[§B8] list a list of references to domain services or objectsxref:section-b/chapter-11.adoc[§B11] object a domain object instance (or a service, which is a singleton object) xref:section-c/chapter-14.adoc[§C14].4 object-property a domain object property xref:section-c/chapter-16.adoc[§C16].4 object-collection a domain object collection xref:section-c/chapter-17.adoc[§C17].5 object-action a domain object action xref:section-c/chapter-18.adoc[§C18].2 action-result result of invoking a domain object action xref:section-c/chapter-19.adoc[§C19].4 type-list a list of domain types xref:section-d/chapter-21.adoc[§D21].2 domain-type a domain type xref:section-d/chapter-22.adoc[§D22].2 property-description a domain property's description § D23.2 collection-description a domain collection's description xref:section-d/chapter-24.adoc[§D24].2 action-description a domain action's description.
xref:section-d/chapter-25.adoc[§D25].2 action-param-description an action parameter's description xref:section-d/chapter-26.adoc[§D26].2 type-action-result result of invoking a domain type action xref:section-d/chapter-27.adoc[§D27]. error An error was generated, xref:section-b/chapter-10.adoc[§B10].
...
This means that the client cannot currently use this parameter to ensure that, for example, v1 of a view model is returned rather than v2. Support for content negotiation through the "x-ro-domain-type" parameter in this way is likely to be introduced in a future version of the spec, see xref:section-e/chapter-34.adoc[§E34].1. If the client does elect to specify "profile" parameters, then it should take care to always include the error profile.
...
If the implementation supports the formal metamodel scheme §3.1.2, then each of these datatypes has a corresponding pre-defined domain type resource xref:section-d/chapter-21.adoc[§D21].3. Support for blobs and clobs is an optional capability, and is discussed further in §3.3.
...
For example:
urn.org.restfulobjects:rels/details;property=\"deliveryOption\" is the rel value of a link to property details resource, xref:section-c/chapter-16.adoc[§C16].1. The table below lists all the supported rel values defined by Restul Objects.
For brevity the "urn:org.restfulobjects:rels/" prefix is abbreviated to ".../".
rel Parameters Description .../action Description of an action xref:section-d/chapter-25.adoc[§D25], as linked from a domain type xref:section-d/chapter-22.adoc[§D22] .../action-param Description of an action parameter xref:section-d/chapter-26.adoc[§D26], as linked from an action resource xref:section-d/chapter-25.adoc[§D25] .../add-to; collection=\"collectionName\" Add to a domain object collection xref:section-c/chapter-17.adoc[§C17].2, xref:section-c/chapter-17.adoc[§C17].3 .../attachment; property=\"propertyName\" An attachment for a property value; see §3.3. .../choice; property=\"propertyName\"
- or - action=\"actionName\"; param=\"paramName\" A domain object (or scalar value) acting as a choice for a property xref:section-c/chapter-16.adoc[§C16].4.1 or an action parameter xref:section-c/chapter-18.adoc[§C18].2.1 .../clear property=\"propertyName\" Clear a domain object property xref:section-c/chapter-16.adoc[§C16].3 .../collection Description of a collection xref:section-d/chapter-24.adoc[§D24], as linked from a domain type xref:section-d/chapter-22.adoc[§D22] .../default; action=\"actionName\"; param=\"paramName\" A domain object (or scalar value) acting as a default for an action parameter .../delete Link to delete a domain object xref:section-c/chapter-14.adoc[§C14].3 .../details; property=\"propertyName\"
- or - collection=\"collectionName\"
- or - action=\"actionName\" Details of a property xref:section-c/chapter-16.adoc[§C16].1, collection xref:section-c/chapter-17.adoc[§C17].1 or action xref:section-c/chapter-18.adoc[§C18].1, as linked from a domain object xref:section-c/chapter-14.adoc[§C14].1 or domain service xref:section-c/chapter-15.adoc[§C15].1. .../domain-type Link to a domain type xref:section-d/chapter-22.adoc[§D22]. .../domain-types Link to the catalogue of domain types available in the system xref:section-d/chapter-21.adoc[§D21] .../element Link to a domain object xref:section-c/chapter-14.adoc[§C14] from a list returned by an action xref:section-b/chapter-11.adoc[§B11]. .../element-type The domain type xref:section-d/chapter-22.adoc[§D22] which represents the element of a list or collection .../invoke; action=\"actionName\"
...
.../persist Link to persist a proto-persistent object xref:section-b/chapter-09.adoc#_9-1-http-post[§B9.1] .../property Description of a property xref:section-d/chapter-23.adoc[§D23], as linked from a domain type xref:section-d/chapter-22.adoc[§D22] .../remove-from; collection=\"collectionName\" Remove from a domain object collection, xref:section-c/chapter-17.adoc[§C17].4 .../return-type The domain type xref:section-d/chapter-22.adoc[§D22] which represents the (return) type of a property, collection, action or param .../service; serviceId=\"serviceId\" A domain service, xref:section-c/chapter-15.adoc[§C15].1 .../services The set of available domain services, xref:section-b/chapter-07.adoc#_7-1-http-get[§B7] .../update Link to modify all properties of a domain object xref:section-c/chapter-14.adoc[§C14].2. .../user The current user, xref:section-b/chapter-06.adoc#_6-1-http-get[§B6.1] .../value; property=\"propertyName\"
- or - collection=\"collectionName\" Link to an object xref:section-c/chapter-14.adoc[§C14] that is the value of a property xref:section-c/chapter-16.adoc[§C16].1 or held within a collection xref:section-c/chapter-17.adoc[§C17].1. .../version Version of the spec and implementation, §B8.1
...
Future versions of this specification may define a syntax to allow clients to request eager loading of links, xref:section-e/chapter-34.adoc[§E34].4.
...
} In addition, the invocation of a query-only action (using GET xref:section-c/chapter-19.adoc[§C19].1) will also have a "self" link, this time linking back to the action.
...
The second is the representation returned by any action invoked by either a PUT or POST method xref:section-c/chapter-19.adoc[§C19].2, xref:section-c/chapter-19.adoc[§C19].3. These have no self link, to minimize the risk of a client repeating the action and inadvertently causing side effects in the system.
...
If a query-only action is being invoked through GET xref:section-c/chapter-19.adoc[§C19].1, and all arguments are scalar values, then the action may be invoked using simple ‘param=value’ arguments.
For example:
...
This form of arguments also cannot be used when updating multiple properties xref:section-c/chapter-14.adoc[§C14].2. For these cases the ‘Formal’ mechanism must be used §3.1.2.
...
* resources that require a single value (xref:section-c/chapter-16.adoc[§C16].2, xref:section-c/chapter-17.adoc[§C17].2) take a single argument node;
* the action resource methods (xref:section-c/chapter-19.adoc[§C19].1, xref:section-c/chapter-19.adoc[§C19].2, xref:section-c/chapter-19.adoc[§C19].3) take a map of argument nodes;
* the update of multiple properties xref:section-c/chapter-14.adoc[§C14].2 takes a map of argument nodes (the arguments representing the property values)
* the persist of a new object (xref:section-b/chapter-09.adoc[§B9]) also takes a map-like structure but in this case the map is based on a cut-down version of the object representation, xref:section-c/chapter-14.adoc[§C14].4) Treating property values and action arguments in the same way simplifies matters, but it does require that action resources provide a unique name for each of their arguments (rather than merely by a position, as in a list).
...
Action resources (xref:section-c/chapter-19.adoc[§C19].2, xref:section-c/chapter-19.adoc[§C19].3) and the PUT Object resource xref:section-c/chapter-14.adoc[§C14].2 accept arguments only in map form.
...
The set of argument choices for a parameter can be found by obtaining a representation of the action resource xref:section-c/chapter-18.adoc[§C18].1.1. For example, the list of categories could be returned as:
{ "category": { ...
"choices": [
...
} } Note that the spec does not currently support obtaining the set of choices of one parameter based on another; see xref:section-e/chapter-34.adoc[§E34].5 for discussion on proposals for this as a future feature.
